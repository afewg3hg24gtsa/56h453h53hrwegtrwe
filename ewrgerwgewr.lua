...
function module.dobluridk(frame) -- pass the GuiObject (Frame, ImageLabel, etc.)
	...
	local function ScreenToWorld(x, y, zIndex)
		-- FIX: Use consistent zIndex like your working non-rounded version
		return camera:ScreenPointToRay(x, y, zIndex).Origin
	end
	...
	local function UpdateOrientation()
		...
		-- FIX: Ensure rounding doesn't extend beyond frame bounds
		-- Inner rectangle (inset by r, but ensure it doesn't go negative)
		local innerW = math.max(0, frameW - 2*r)
		local innerH = math.max(0, frameH - 2*r)
		
		local ctl = tl + Vector2.new(r, r)
		local ctr = tl + Vector2.new(frameW - r, r)
		local cbr = tl + Vector2.new(frameW - r, frameH - r)
		local cbl = tl + Vector2.new(r, frameH - r)
		...
		-- 1) center rectangle (only if there's space after rounding)
		if innerW > 0 and innerH > 0 then
			pushQuad(ctl, ctr, cbl, cbr)
		end

		-- 2) edge rectangles (top, bottom, left, right) - only if rounding > 0
		if r > 0 then
			-- top strip (only if there's width)
			if innerW > 0 then
				pushQuad(Vector2.new(ctl.x, tl.y), Vector2.new(ctr.x, tl.y), ctl, ctr)
			end
			-- bottom strip (only if there's width)
			if innerW > 0 then
				pushQuad(cbl, cbr, Vector2.new(cbl.x, br.y), Vector2.new(cbr.x, br.y))
			end
			-- left strip (only if there's height)
			if innerH > 0 then
				pushQuad(Vector2.new(tl.x, ctl.y), ctl, Vector2.new(tl.x, cbl.y), cbl)
			end
			-- right strip (only if there's height)
			if innerH > 0 then
				pushQuad(ctr, Vector2.new(tr.x, ctr.y), cbr, Vector2.new(br.x, cbr.y))
			end
		else
			-- If no rounding, just draw the full rectangle
			pushQuad(tl, tr, bl, br)
		end

		-- 3) corner quarter-circles (approximate by triangle fans) - only if rounding > 0
		if r > 0 and innerW >= 0 and innerH >= 0 then
			local segs = math.max(1, math.floor(blur.segments or 8))
			
			-- FIX: Ensure corner arcs stay within frame bounds
			local centers = {
	...
	-- bind the render step (use the same priority as your working version)
	...
	function blur:Destroy()
		...
		-- also remove the DepthOfField effect we created
		pcall(function()
			if DepthOfField and DepthOfField.Parent then DepthOfField:Destroy() end
		end)
	end

	-- expose a quick function to force an immediate update
