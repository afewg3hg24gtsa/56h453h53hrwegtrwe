...
function module.dobluridk(frame) -- pass the GuiObject (Frame, ImageLabel, etc.)
	...
	local function ScreenToWorld(x, y, zIndex)
		-- zIndex small offset from camera to avoid clipping (same as original)
	...
	local function UpdateOrientation()
		...
		if r > 0 then
			...
			-- if no rounding, just fill the full rectangle using the four corners
			-- we already drew center as full rect when r == 0
		end
